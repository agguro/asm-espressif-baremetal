...
#              (Cleaned, structured for lib.a, using Linux-kernel style 
#               local .L labels, explicit non-executable stack note, 
#               100% stack-allocated inline nanosleep blocks via .equ, and 
#               immediate auto-exit after kernel execution handoff).
#
...
# =====================================================================
# Assembler Constants - Sleep Durations & Target Memory Map
# =====================================================================
.equ SLEEP_100_NS, 100000000
.equ SLEEP_200_NS, 200000000
.equ TARGET_SRAM_ADDR, 0x40380000   # ESP32-C3 internal SRAM execution base address
...
.section .rodata, "a", @progbits
    ...
    msg_running:    .string "[HOST] Kernel execution started! Exiting loader session.\n"
    len_running   = . - msg_running - 1

    msg_close:      .string "[HOST] Port successfully closed. Program exits.\n"
    ...
    run_packet:
        ...
        .4byte TARGET_SRAM_ADDR       # Entry point address in SRAM (driven by .equ constant)
        .byte 0xC0                  
    run_len = . - run_packet
...
.section .data, "aw", @progbits
    ...
    mem_begin_packet:
        ...
        .4byte TARGET_SRAM_ADDR       # Target destination address in SRAM (driven by .equ constant)
        .byte 0xC0                  
...
_start:
    ...
    jl .Lcheck_kernel_file          # If so, proceed with defaults
...
.Lcheck_kernel_file:
    ...
    js .Lload_fallback_kernel       # If negative (error/not found), jump to fallback routine
    ...
    jmp .Lstart_host_sequence       # Proceed to serial communication flow

# ---------------------------------------------------------------------
# Fallback Routine: Load embedded fallback binary if kernel file is missing
# ---------------------------------------------------------------------
.Lload_fallback_kernel:
...
.Lstart_host_sequence:
    ...
    js .Lopen_failed                # Jump to error handler if open failed
    ...
    # Inline 100ms stack-allocated nanosleep
    subq $16, %rsp                  # Allocate timespec on stack
    movq $0, (%rsp)                 # tv_sec = 0
    movabsq $SLEEP_100_NS, %rax
    movq %rax, 8(%rsp)              # tv_nsec = SLEEP_100_NS
    movq $35, %rax                  # Syscall 35: sys_nanosleep
    movq %rsp, %rdi                 # %rdi points to stack timespec
    xorq %rsi, %rsi                 # NULL remaining time
    syscall
    addq $16, %rsp                  # Clean up stack timespec
    ...
    addq $8, %rsp                   # Restore stack pointer (ioctls allocation)
    
    # Inline 200ms stack-allocated nanosleep for hardware stabilization
    subq $16, %rsp
    movq $0, (%rsp)
    movabsq $SLEEP_200_NS, %rax
    movq %rax, 8(%rsp)
    movq $35, %rax
    movq %rsp, %rdi
    xorq %rsi, %rsi
...
.Ldrain_logs:
    ...
    jg .Ldrain_logs                 # Loop until input buffer is completely drained
...
.Lsend_sync_loop:
    ...
    js .Lsync_failed                # If counter reaches zero, jump to synchronization error handler
    ...
    jle .Lsend_sync_loop            # If no data or error, retry synchronization packet
    ...
    jne .Lsend_sync_loop            # If acknowledgment does not match, retry loop
...
.Lread_attach_retry:
    ...
    jle .Lread_attach_retry         # Retry reading until response arrives
...
.Lread_mem_begin_retry:
    ...
    syscall
    cmpq $0, %rax                   # Check if response was received
    jle .Lread_mem_begin_retry      # Retry reading acknowledgment
...
.Lread_mem_data_retry:
    ...
    syscall
    cmpq $0, %rax                   # Check if response was received
    jle .Lread_mem_data_retry       # Retry reading data transfer acknowledgment
    ...
    # Inline 100ms stack-allocated pause for execution handoff
    subq $16, %rsp
    movq $0, (%rsp)
    movabsq $SLEEP_100_NS, %rax
    movq %rax, 8(%rsp)
...
.Ldrain_final_ack:
    ...
    jg .Ldrain_final_ack            # Loop until input buffer is fully cleared
    ...
    # Print kernel execution started message and exit immediately
    movq $1, %rax                   # Syscall number 1: sys_write
    movq $1, %rdi                   # File descriptor 1: stdout
    leaq msg_running(%rip), %rsi    # Pointer to running message string
    movq $len_running, %rdx         # Length of running message
    syscall
    jmp .Lclose_and_exit            # Jump straight to clean exit without listening loop
...
.Lclose_and_exit:
    ...
    jmp .Lprogram_exit              # Jump to final program termination
...
.Lsync_failed:
    ...
    jmp .Lclose_and_exit            # Jump to cleanup and exit routine
...
# =====================================================================
# Utility Subroutines - Serial I/O and Checksums
# (Core I/O primitives kept for future lib.a extraction)
# =====================================================================
send_packet_safe:
...
.Lcalc_sum:
    ...
    jz .Lsave_checksum              # If zero, jump out of checksum loop
    ...
    jmp .Lcalc_sum                  # Repeat checksum calculation loop
...
.Lsave_checksum:
    ...
    call .Lstream_loop_escaped      # Stream escaped header bytes
    ...
    call .Lstream_loop_escaped      # Stream escaped payload bytes
...
.Lstream_loop_escaped:
    ...
    jmp .Lnext_byte                 # Proceed to next byte
...
.Lnext_byte:
    ...
    jmp .Lstream_loop_escaped       # Repeat streaming loop

.Lstream_done:
    ret                             # Return from subroutine

# =====================================================================
# Program Total Size Directive & Non-Executable Stack Note
# =====================================================================
.size _start, . - _start
