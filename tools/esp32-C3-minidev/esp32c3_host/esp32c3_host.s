...
#              bytes directly inside the code section.
#
...
# =====================================================================
.section .rodata
    kernel_path:   .string "esp32c3_kernel.bin"
    ...
    msg_fallback:  .string "[HOST] WARNING: 'esp32c3_kernel.bin' not found! Using embedded fallback kernel.\n"
    ...
    # ESP32-C3 Serial Protocol: Execution Vector Trigger Packet (MEM_END)
    run_packet:
        .byte 0xC0                    
...
_start:
    # -----------------------------------------------------------------
    # Step 0: Check if 'esp32c3_kernel.bin' exists on disk
    # -----------------------------------------------------------------
    movq $2, %rax                 # Syscall number 2: sys_open
    leaq kernel_path(%rip), %rdi  # Pointer to kernel filename string
    movq $0, %rsi                 # Flags: O_RDONLY (Read-only mode)
    movq $0, %rdx                 # Mode: 0 (Not used for opening existing files)
    ...
    syscall                       # Execute system call
    movq %rax, %r9                # Save the exact number of bytes read into %r9
    
    # Close the custom kernel file descriptor
    movq $3, %rax                 # Syscall number 3: sys_close
    movq %r8, %rdi                # File descriptor argument
    syscall                       # Execute system call
    
    movq %r9, kernel_size(%rip)   # Store actual kernel size variable
    jmp start_host_sequence       # Proceed directly to serial communication flow
...
load_fallback_kernel:
    ...
    syscall                       # Print warning message to terminal
...
# ---------------------------------------------------------------------
# Step 1: Open Serial Communication Port (/dev/ttyACM0)
# ---------------------------------------------------------------------
start_host_sequence:
    movq $2, %rax                 # Syscall number 2: sys_open
    leaq dev_path(%rip), %rdi     # Pointer to device path string ("/dev/ttyACM0")
...
open_failed:
    ...
    movq $1, %rax                 # Syscall number 1: sys_write
    movq $2, %rdi                 # File descriptor 2: stderr
    leaq dev_path(%rip), %rsi     # Pointer to device path string
...
# =====================================================================
# BSS Section - End of Program Declarations
# =====================================================================
.section .bss
